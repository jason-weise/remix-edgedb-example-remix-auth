...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly create remix-stack-dd97
  fly create remix-stack-dd97-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-stack-dd97
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-stack-dd97-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'remix-stack-dd97-staging' does not match app name in config file 'remix-stack-dd97'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name remix-stack-dd97-db
  fly postgres attach --postgres-app remix-stack-dd97-db --app remix-stack-dd97

  fly postgres create --name remix-stack-dd97-staging-db
  fly postgres attach --postgres-app remix-stack-dd97-staging-db --app remix-stack-dd97-staging
  ```
